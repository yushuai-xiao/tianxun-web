          ...
          <el-card class="box-card">
            ...
            <div class="table-content">
              <el-table :data="tableData" border style="width: 100%">
                ...
                <el-table-column prop="reason" label="出差原因" />
...
export default defineComponent({
  ...
  setup() {
    ...
    const getList = async () => {
      if (queryAll.value) {
        // const listResult = await getBusinessList(formList.value)
        getBusinessList(formList.value).then((res) => {
          if (res.code === 200) {
    ...
    const approval = (id: string) => {
      if (queryAll.value) {
        businessStatus({ id, status: '2' }).then((res) => {
          if (res.code === 200) {
            ElMessage({
              message: res.message,
              type: 'success'
    ...
    const disapproval = (id: string) => {
      if (queryAll.value) {
        businessStatus({ id, status: '1' }).then((res) => {
          if (res.code === 200) {
            ElMessage({
              message: res.message,
              type: 'success'
